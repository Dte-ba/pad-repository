 ...
 * Get file with flag `ready`
 */
Metadata.prototype.getFilesToRemove = function() {
	return this.getFilesFor(status.missing);
};

/**
 * Get file with `flag`
 */
Metadata.prototype.getFilesFor = function(flag) {
	var self = this
	  , res = [];
    
  for (var idx in self.filesStats) {
		if (self.filesStats[idx] == flag) {
			res.push(idx);
		}
...
/**
 * Remove a filename or an array of them.
 *
 * @param {String|Array} filename
 * @param {Boolean} cascade
 */
Metadata.prototype.removeFile = function(file, cascade) {
	var self = this
	var uids = [];

	cascade = (cascade === undefined) ? true : cascade;

	if (false === (file instanceof Array)) {
		file = [ file ];
	}

	file.forEach(function(f) {
		var fidx = self.getIndexFile(f);

		var ps = self.packages.filter(function(p){
			var ii = self.getIndexPackage(p);
			return self.content[ii].fileIndex == fidx;
		});

		if (ps.length > 0) {
			var uid = _.first(ps);

			var idp = self.getIndexPackage(uid);

			// delete from the files array
			self.files.splice(fidx, 1);

			// delete from the packages
			self.packages.splice(idp, 1);
	...
	self.files.forEach(function(f){
		if (self.getFileStats(f) === undefined) {
				self.setFileStats(f, status.ready);
		}
	});
 ...
 * @param {String} word
 */
Metadata.prototype.find = function(word, area) {
	var self = this;

	var pkgIndexs = self.lexer.getMatches(word);

	var res = pkgIndexs.map(function(idx){
			return self.content[idx];
	});

	if ( area === undefined ) { return res; }

	res = res.filder(function(p){
		return p.content.area === are;
	});
...
/**
 * Get a package from index
 *
 * @param {Int} index
 */
Metadata.prototype.getInfoPackageFrom = function(index) {
	var self = this;
	
	var pkg = self.content[index];
	pkg.filename = self.files[pkg.fileIndex];

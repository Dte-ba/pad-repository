...
var self = this
	...
	self.load(ops, function(err){
		...
		if (fremove.length > 0) {
			var uids = self.metadata.removeFile(fremove, true);
			
			uids.forEach(function(u){
				self.cache.removeContent(u);
			});
 ...
 * @param {Function} fn
 */
Repository.prototype.find = function(word, area, fn) {
	var self = this;
  ...
  self.refresh({}, function(err){
  	...
  	try {
  		var res = self.metadata.find(word, area);
  		fn && fn (null, res);
  	} catch (e) {
			return self.resideError(e, fn);
  	}
...
/**
 * Get stats from all packages
 *
 * @param {Function} fn
 */
Repository.prototype.getStats = function(fn) {
	var self = this;
  ...
  self.refresh({}, function(err){
  	...
  	try {
  		...
  		var ps = Object.keys(pkgs).map(function(c){
  			return pkgs[c]
  		});

  		var cts = ps.map(function(p){
  			return p.content;
  		});
...
/**
 * Restore cache application
 *
 * @param {Function} fn
 */
Repository.prototype.restore = function(fn) {
	var self = this;
  
  // remove cache folder
  rmdir(self.config.cachePath, function(err){
  	if (err) {
  		return self.resideError(err, fn);
  	}

  	// rewrite repository.json
  	var rj = path.join(self.path, 'repository.json');

  	fs.writeFileSync(rj, JSON.stringify({}));
  	...
  	self.refresh({}, function(err) {
			...
			console.log('restored repository');

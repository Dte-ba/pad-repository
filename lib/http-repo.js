...
var HttpRepo = module.exports = function(dir, app, serverHeadRoute) {
	...
	app.get('/metadata/words', function(req, res) {
		self.getWords({}, function(err, data){
			if (err) {
	...
	app.get('/package/:uid/content', function(req, res, next) {
		...
		function send(data) {
			if (data.type === 'file') {
				...
				if (!self.contentRegistered) {
					app.use(express.static(path.join(self.config.cachePath, 'content')));
					app.use(express.static(path.join(self.config.cachePath, 'content', uid)));

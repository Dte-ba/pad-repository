...
/**
 * Remove the `content` of a package.
 *
 * @param {Object} content
 * @param {Int} package index
 */
Lexer.prototype.remove = function(content, pindex) {
  var self = this;
...
Lexer.prototype.getWords = function(ops) {
  ...
  var matches = self.getMatches(ops.filter, ops.forceLevel);

  var added = [];

  matches.forEach(function(m) {

    for (var wa in self.wordInfo) {
      var wi = self.wordInfo[wa]

      if ( 
         wi.matches[wordLevel.area].indexOf(m) !== -1
      || wi.matches[wordLevel.axis].indexOf(m) !== -1
      || wi.matches[wordLevel.block].indexOf(m) !== -1
      || wi.matches[wordLevel.tag].indexOf(m) !== -1
      ) {
        var fw = wi.words[0];

        if (added.indexOf(wa) === -1) {
          added.push(wa);
          res.push(self.words[fw]);  
        }
